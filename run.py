...
def run():
    ...
    if env["SHOW_CORNER"]:
        ...
        none_corner = [item for item in road_section if
                            WEEK_CORNER_ANGLE_MIN > item['adjusted_steering_angle']
                            and item['section_type'] != 'straight']
        ...
        if week_sections:
            ax.plot([], [], color="blue", label='week')
            for section in week_sections:
                ax.plot(*zip(*section), color="blue", linewidth=2)
        if medium_sections:
        ...
        if none_sections:
            ax.plot([], [], color="orange", label='none')
        ...
        print(f"week: {week_corner_distance}m, medium: {medium_corner_distance}m, strong: {strong_corner_distance}m, none: {none_corner_distance}m, straight: {straight_distance}m")
