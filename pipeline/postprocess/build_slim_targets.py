...
def prefetch_cities(coord_keys):
    """座標キーのリストからユニークなものだけAPIで取得し、キャッシュに格納する。"""
    unique = [k for k in set(coord_keys) if k not in _city_cache]
    if not unique:
        return
    print(f"  市区町村名を取得中... {len(unique)}件 (キャッシュ済み{len(coord_keys)-len(unique)}件)", flush=True)
    done = 0
    failed = 0
    ...
    with ThreadPoolExecutor(max_workers=CITY_CONCURRENCY) as pool:
        futures = {pool.submit(fetch_one, k): k for k in unique}
        for f in as_completed(futures):
            done += 1
            if not f.result():
                failed += 1
            if done % 200 == 0:
                print(f"    {done}/{len(unique)} ({failed} failed)", flush=True)

    if failed:
        print(f"  完了 ({failed}/{len(unique)}件 取得失敗)", flush=True)
