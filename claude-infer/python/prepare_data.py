...
def build_geometry_lookup(pref_code: str = None) -> dict:
...
def fetch_samples_from_db(pref_code: str = None) -> list:
...
def prepare_dataset(pref_code: str = None, check_images: bool = True):
    """学習用データセットを準備"""
    print("=== データセット準備開始 ===")

    # 1. geometry_lookupを構築
    print("\n1. Geometry lookup構築中...")
    lookup = build_geometry_lookup(pref_code)
    geometry_map = lookup["geometry_map"]
    valid_coords = lookup["valid_coords"]

    # 2. DBからサンプル取得
    print("\n2. DBからサンプル取得中...")
    rows = fetch_samples_from_db(pref_code)
...
def main():
    ...
    parser.add_argument("--pref", type=str, help="都道府県コード（例: 21）")
